...
def returnCAM(feature_conv, weight_softmax):
    ...
    cam0 = weight_softmax[0].dot(feature_conv.reshape((nc, h * w)))
    cam1 = weight_softmax[1].dot(feature_conv.reshape((nc, h * w)))
    cam = np.zeros((h * w, 1))
    softmax_temp = np.zeros((2, 1))
    for j in range(0, h * w):
        exp0 = np.exp(cam0[j])
        exp1 = np.exp(cam1[j])
        exp_sum = exp0 + exp1
        softmax_temp[0] = exp0 / exp_sum
        softmax_temp[1] = exp1 / exp_sum
        cam[j] = softmax_temp[1]
        # cam[j] = np.amax(softmax_temp)
        # print(cam[j])
    cam = cam.reshape(h, w)
    # cam = cam - np.min(cam)
